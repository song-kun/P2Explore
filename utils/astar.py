...
class grid_path(AStar):
    ...
    #using (y,x) as input
    def __init__(self, grid_map,distance_map,start_point,end_point,free_flag = 0):
    ...
    def distance_between(self, n1, n2):
        if n1[0] == n2[0] or n1[1]  == n2[1]:
            return 1
        elif abs(n1[0] - n2[0])==1 and abs(n1[1] - n2[1])==1:
            return 1.414
        else:
            return 2.2361
    ...
    def neighbors(self, node):
        ...
        # (x-2,y-1),(x+2,y-1),(x-2,y+1),(x+2,y+1),(x-1,y-2),(x-1,y+2),(x+1,y-2),(x+2,y+2)]
        # if 0 <= nx < self.width and 0 <= ny < self.height and self.grid_map[nx, ny] ==0  ]
        return[(nx, ny) for nx, ny in[(x, y - 1), (x, y + 1), (x - 1, y), (x + 1, y), (x + 1, y),(x-1, y - 1), (x - 1, y + 1), (x + 1, y -1), (x + 1, y +1)]
        if 0 <= nx < self.width and 0 <= ny < self.height and self.grid_map[nx, ny] ==self.free_flag  ]
