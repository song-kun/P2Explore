...
class BasicAIC2d(nn.Module):
    def __init__(self, channel, kernel=(3, 5, 7), dilation=(1, 1, 1), residual=True):
        ...
        self.conv_mx = nn.Conv2d(channel, 2 * self.n, (1, 1), stride=1, padding=0, bias=False, dilation=1)
        ...
        for _idx in range(self.n):
            ...
            d = dilation[_idx]
            p = k // 2 * d
            self.conv_1xk.append(nn.Conv2d(c, c, (1, k), stride=1, padding=(0, p), bias=True, dilation=(1, d)))
            self.conv_kx1.append(nn.Conv2d(c, c, (k, 1), stride=1, padding=(p, 0), bias=True, dilation=(d, 1)))
    ...
    def forward(self, x):
        ...
        for i in range(self.n):
            ...
            mx_y, mx_x = torch.split(torch.squeeze(mx_list[i], dim=2), 1, dim=1)  # 3 x (BS, 1, c, D, H, W)
        ...
        y = F.relu(y_y + x, inplace=True) if self.residual else F.relu(y_y, inplace=True)
        return y

class an_unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, n=2, ks=3, stride=1, padding=1, kernel = (3,5,7), dilation=(1, 1, 1)):
        super(an_unetConv2, self).__init__()
        self.n = n
        self.ks = ks
        self.stride = stride
        self.padding = padding
        ...
        if is_batchnorm:
            for i in range(1, n + 1):
                conv = nn.Sequential(BasicAIC2d(in_size, kernel=kernel, dilation=dilation, residual=True),
...
class FPUNet(nn.Module):
    def __init__(self, in_channels=3, n_classes=1, feature_scale=4, is_deconv=True, is_batchnorm=True, is_ds=True):
        ...
        # downsampling
        self.conv00 = an_unetConv2(self.in_channels, filters[0], self.is_batchnorm)
        self.maxpool0 = nn.MaxPool2d(kernel_size=2)
        self.conv10 = an_unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2)
        self.conv20 = an_unetConv2(filters[1], filters[2], self.is_batchnorm)
        self.maxpool2 = nn.MaxPool2d(kernel_size=2)
        self.conv30 = an_unetConv2(filters[2], filters[3], self.is_batchnorm)
        self.maxpool3 = nn.MaxPool2d(kernel_size=2)
        self.conv40 = an_unetConv2(filters[3], filters[4], self.is_batchnorm)
        ...
        #ouput the probability of the class
